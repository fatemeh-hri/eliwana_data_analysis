...
DEFAULT_END_DATETIME = "2026-06-15 13:00:00"
...
def detect_intersections_hybrid(lane_gdf):
    ...
    fallback_intersections = detect_intersections_from_dxf_lanes(
        lane_gdf,
        # dwithin_m=3,
        dwithin_m=4,
        cluster_buffer_m=10,
        ...
        polygon_buffer_m=10,
        min_pair_count=12,
        min_distinct_lanes=6,
        min_direction_bins=2,
    )
...
def classify_area_categories(gdf, lane_gdf, intersection_gdf):
    ...
    lane_union = lanes_source.geometry.union_all()
    ...
    def nearest_lane_attributes(point):
        ...
        return pd.Series({
            "DISTANCE_TO_LANE_M": distances.loc[nearest_idx],
            "polyline_id": nearest_lane.get("polyline_id"),
            "lane_layer": nearest_lane.get("layer"),
            "lane_vertex_count": nearest_lane.get("vertex_count")
        })

    lane_attributes = points_source.geometry.apply(nearest_lane_attributes)
    classified = pd.concat([classified, lane_attributes], axis=1)
    ...
    # WL away from haul roads/intersections is treated as normal loading operation noise.
    classified = classified[
        ~(
            (classified["TYPE"] == "HOV") &
            (classified["PREFIX"] == "WL") &
            (classified["AREA_CATEGORY"] == "dynamic area")
        )
    ].copy()
...
#     intersection_points = gdf_plot[
#         gdf_plot["AREA_CATEGORY"] == "intersection"
...
#     for _, near_miss_row in near_miss_points.iterrows():
