...
DEFAULT_START_DATETIME = "2026-05-09 09:19:01"
DEFAULT_END_DATETIME = "2026-05-09 10:19:01"
...
def load_nearby_machine_data(start_date, end_date, max_distance):
    ...
    query = f"""
        WITH hme AS (
            ...
            AND LEFT(MACHINE, 2) IN ('DZ', 'WL', 'WD', 'DT')
        ),
        ...
        matches AS (
            ...
            FROM hme h
            JOIN lv l
            ON DATE_TRUNC('SECOND', h."TIMESTAMP") =
                DATE_TRUNC('SECOND', l."TIMESTAMP")
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            "Eliwana Dynamic Area – Nearby Machine Detection",
            style={
                "padding": "10px",
                "fontSize": "20px",
                "fontWeight": "bold"
...
def update_map(selected_machines, time_limit, distance_threshold, start_date, end_date):
    ...
    gdf_plot = process_pair_data(df)
    print(gdf_plot[gdf_plot["TIMESTAMP"] == '2026-05-09 09:51:20'])
    if gdf_plot.empty:
        return []
    ...
    # print(grouped[('HME', 'WL')][(grouped[('HME', 'WL')]["TIMESTAMP"] >= '2026-05-09 09:30:20') & (grouped[('HME', 'WL')]["X"] == 512830.13) & (grouped[('HME', 'WL')]["Y"] == 7510692.85)])
    # max_markers_per_group = 300
    ...
    for machine_type, group in grouped.items():
        ...
        # if len(group) > max_markers_per_group:
        #     group = group.iloc[
        #         ::max(1, len(group) // max_markers_per_group)
        #     ]
