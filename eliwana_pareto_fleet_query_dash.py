...
def classify_area_categories(gdf):
    ...
    if lanes_source.empty:
        classified["DISTANCE_TO_LANE_M"] = None
        classified["polyline_id"] = None
        classified["lane_layer"] = None
        classified["lane_vertex_count"] = None
        classified["ON_LANE"] = False
        classified["AREA_CATEGORY"] = "Dynamic Area"
        return classified

    points_source = classified.to_crs(SOURCE_CRS).copy()
    points_source["SOURCE_ROW_ID"] = range(len(points_source))

    lane_columns = ["polyline_id", "lane_layer", "lane_vertex_count", "geometry"]
    ...
    classified["DISTANCE_TO_LANE_M"] = nearest["DISTANCE_TO_LANE_M"].reindex(classified.index).to_numpy()
    classified["polyline_id"] = nearest["polyline_id"].reindex(classified.index).to_numpy()
    classified["lane_layer"] = nearest["lane_layer"].reindex(classified.index).to_numpy()
    classified["lane_vertex_count"] = nearest["lane_vertex_count"].reindex(classified.index).to_numpy()

    classified["ON_LANE"] = classified["DISTANCE_TO_LANE_M"] <= LANE_BUFFER_METRES
    classified["AREA_CATEGORY"] = "Dynamic Area"
    classified.loc[classified["ON_LANE"], "AREA_CATEGORY"] = "Haul Road"

    debug_log("Area classification completed", f"{len(classified)} rows")
...
def get_pareto_payload(start_date, end_date):
    ...
    keep_columns = [
        ...
        "AREA_CATEGORY", "ASSET_AREA_CATEGORY", "DISTANCE_METRES", "IS_NEAR_MISS",
        "IS_VEHICLE_INTERACTION", "MOVING_STATUS", "FLEET_SIZE", "PAIR_FLEET_SIZE",
        "IS_LOADING_OPERATION", "DISTANCE_TO_LANE_M", "polyline_id", "lane_layer"
    ]
...
app.layout = html.Div(
    ...
    children=[
        ...
        dcc.Checklist(
            ...
            options=[{
                "label": "Hide loading operation interactions: EX/WL in Dynamic Area and Haul Road",
                "value": "hide_loading",
            }],
            value=[],
...
def update_pareto(payload, hide_loading_value):
    ...
    area_fig = build_pareto_chart_from_summary(
        ...
        "Pareto by Dynamic Area / Haul Road",
    )
