...
EXPORT_XLSX_PATH = r"eliwana_hme_lv_pareto_output_with_speed.xlsx"
...
def add_historical_risk_analysis(df):
    ...
    lv_moving = pd.to_numeric(out["PAIR_IS_MOVING"], errors="coerce").fillna(0).eq(1)
    hme_moving = pd.to_numeric(out["IS_MOVING"], errors="coerce").fillna(0).eq(1)
    ...
    # out.loc[is_intersection & hme_moving & ~lv_moving, "HIERARCHY_ASSESSMENT"] = (
    #     "HME moving and LV stationary"
    # )
    # out.loc[is_intersection & lv_moving, "HIERARCHY_ASSESSMENT"] = (
    #     "LV moving in intersection"
    # )
    out.loc[is_intersection & hme_moving, "HIERARCHY_ASSESSMENT"] = (
        "HME moving in intersection"
    )
    out.loc[is_intersection & ~hme_moving & ~lv_moving, "HIERARCHY_ASSESSMENT"] = (
    ...
    out["IS_POTENTIAL_HIERARCHY_BREACH"] = (
        is_intersection & lv_moving & hme_moving
    )
...
WITH base AS (
    SELECT
        ...
        /*
         * Previous timestamp is required for speed calculation.
         */
        LAG("TIMESTAMP") OVER (
...
movement_metrics AS (
    SELECT
        *,
        ...
        /*
         * Normalise ELI and LV machines into the LV asset group.
         * WL, DZ and WD retain their original prefixes.
         */
        CASE
            WHEN MACHINE LIKE 'ELI%' THEN 'LV'
            WHEN LEFT(MACHINE, 2) = 'LV' THEN 'LV'
            ELSE LEFT(MACHINE, 2)
        END AS ASSET_PREFIX,
        ...
        /*
         * Horizontal X/Y distance used to calculate speed.
         * Z is excluded from speed because elevation values may add noise.
         */
        CASE
            WHEN PREV_X IS NULL
              OR PREV_Y IS NULL
        ...
        /*
         * Original 3D movement distance used for IS_MOVING.
         * This preserves the existing interaction counts.
         */
        CASE
            WHEN PREV_X IS NULL
              OR PREV_Y IS NULL
        ...
        /*
         * Elapsed time between consecutive location records.
         */
        CASE
...
movement AS (
    SELECT
        *,
        ...
        /*
         * Speeds above 100 km/h are treated as likely GPS,
         * timestamp or telemetry anomalies.
         *
         * Invalid speed values are returned as NULL only.
         * They do not affect the IS_MOVING classification.
         */
        CASE
        ...
        /*
         * Preserve the original movement definition:
         * 3D displacement greater than 0.5 metres.
         */
        CASE
...
/*
 * HME asset group:
 * Wheel Loaders, Dozers and Wheel Dozers.
 */
hme AS (
    SELECT *
    FROM movement
    WHERE ASSET_PREFIX IN ('WL', 'DZ', 'WD')
),

/*
 * LV asset group:
 * Both LV-prefixed and ELI-prefixed machines are normalised to LV.
 */
lv AS (
    SELECT *
    FROM movement
    WHERE ASSET_PREFIX = 'LV'
),
...
raw_matches AS (
    SELECT
        ...
        h.ASSET_PREFIX AS HME_PREFIX,
        l.ASSET_PREFIX AS LV_PREFIX,

        /*
         * Original source timestamps retain sub-second precision.
         */
        h."TIMESTAMP" AS HME_LOCATION_TIMESTAMP,
        l."TIMESTAMP" AS LV_LOCATION_TIMESTAMP,

        h.TS_SECOND,

        /*
         * Validated speed values.
         */
        h.SPEED_KPH AS HME_SPEED_KPH,
        l.SPEED_KPH AS LV_SPEED_KPH,

        h.SPEED_MPS AS HME_SPEED_MPS,
        l.SPEED_MPS AS LV_SPEED_MPS,

        /*
         * Horizontal movement used for speed calculation.
         */
        h.MOVEMENT_DISTANCE_METRES AS HME_MOVEMENT_METRES,
        l.MOVEMENT_DISTANCE_METRES AS LV_MOVEMENT_METRES,

        /*
         * 3D movement used for IS_MOVING.
         */
        h.MOVEMENT_DISTANCE_3D_METRES AS HME_MOVEMENT_3D_METRES,
        l.MOVEMENT_DISTANCE_3D_METRES AS LV_MOVEMENT_3D_METRES,

        /*
         * Time elapsed between consecutive source records.
         */
        h.TIME_DELTA_SECONDS AS HME_TIME_DELTA_SECONDS,
        l.TIME_DELTA_SECONDS AS LV_TIME_DELTA_SECONDS,
    ...
    INNER JOIN lv l
       ...
       /*
        * Spatial grid pre-filter before calculating exact distance.
        */
       AND l.GRID_X BETWEEN h.GRID_X - 1 AND h.GRID_X + 1
       AND l.GRID_Y BETWEEN h.GRID_Y - 1 AND h.GRID_Y + 1

    /*
     * Current requirement: HME must be moving.
     *
     * To require both HME and LV to be moving, add:
     * AND l.IS_MOVING = 1
     */
    WHERE h.IS_MOVING = 1
),
...
matches AS (
    SELECT
        *,
        ...
        CASE
            WHEN DISTANCE_METRES < {NEAR_MISS_DISTANCE_METRES}
            THEN TRUE
            ELSE FALSE
        END AS IS_NEAR_MISS,
    ...
    WHERE DISTANCE_METRES <= {VEHICLE_INTERACTION_DISTANCE_METRES}
),
...
SELECT
    ...
    e.HME_PREFIX || '-' || e.LV_PREFIX AS ASSET_PAIR,

    hme_fs.FLEET_SIZE AS HME_FLEET_SIZE,
    lv_fs.FLEET_SIZE AS LV_FLEET_SIZE,

    e.TS_SECOND AS "TIMESTAMP",

    /*
     * Original timestamps with sub-second precision.
     */
    e.HME_LOCATION_TIMESTAMP,
    e.LV_LOCATION_TIMESTAMP,

    /*
     * Calculated speed in kilometres per hour.
     */
    ROUND(
        e.HME_SPEED_KPH,
        2
    ...
    /*
     * Calculated speed in metres per second.
     */
    ROUND(
        e.HME_SPEED_MPS,
        2
    ...
    /*
     * Horizontal distance used for speed calculation.
     */
    ROUND(
        e.HME_MOVEMENT_METRES,
        3
    ) AS HME_MOVEMENT_METRES,

    ROUND(
        e.LV_MOVEMENT_METRES,
        3
    ) AS LV_MOVEMENT_METRES,

    /*
     * 3D distance used for movement classification.
     */
    ROUND(
        e.HME_MOVEMENT_3D_METRES,
        3
    ) AS HME_MOVEMENT_3D_METRES,

    ROUND(
        e.LV_MOVEMENT_3D_METRES,
        3
    ) AS LV_MOVEMENT_3D_METRES,

    /*
     * Time elapsed between consecutive location records.
     */
    ROUND(
        e.HME_TIME_DELTA_SECONDS,
        3
    ) AS HME_TIME_DELTA_SECONDS,

    ROUND(
        e.LV_TIME_DELTA_SECONDS,
        3
    ) AS LV_TIME_DELTA_SECONDS,
...
LEFT JOIN fleet_size hme_fs
    ON e.HME_PREFIX = hme_fs.ASSET_PREFIX

LEFT JOIN fleet_size lv_fs
    ON e.LV_PREFIX = lv_fs.ASSET_PREFIX
...
def load_nearby_machine_data_chunked(start_date, end_date, chunk_hours=6):
    ...
    return df.drop_duplicates(
        subset=["HME_MACHINE", "LV_MACHINE", "TIMESTAMP", "DISTANCE_METRES"]
    )
...
def process_pair_data(df):
    ...
    hme_df = df[[
        "HME_MACHINE", "HME_PREFIX", "LV_MACHINE", "LV_PREFIX",
        "TIMESTAMP", "DISTANCE_METRES", "INTERACTION_TYPE", "IS_NEAR_MISS",
        "IS_VEHICLE_INTERACTION", "MOVING_STATUS", "HME_IS_MOVING", "LV_IS_MOVING",
        "HME_X", "HME_Y", "HME_Z", "HME_GEOMETRY", "HME_FLEET_SIZE", "LV_FLEET_SIZE",
        "HME_LOCATION_TIMESTAMP", "LV_LOCATION_TIMESTAMP",
        "HME_SPEED_KPH", "LV_SPEED_KPH", "HME_SPEED_MPS", "LV_SPEED_MPS",
        "HME_MOVEMENT_METRES", "LV_MOVEMENT_METRES",
        "HME_MOVEMENT_3D_METRES", "LV_MOVEMENT_3D_METRES",
        "HME_TIME_DELTA_SECONDS", "LV_TIME_DELTA_SECONDS",
    ...
    hme_df.columns = [
        ...
        "LOCATION_TIMESTAMP", "PAIR_LOCATION_TIMESTAMP",
        "SPEED_KPH", "PAIR_SPEED_KPH", "SPEED_MPS", "PAIR_SPEED_MPS",
        "MOVEMENT_METRES", "PAIR_MOVEMENT_METRES",
        "MOVEMENT_3D_METRES", "PAIR_MOVEMENT_3D_METRES",
        "TIME_DELTA_SECONDS", "PAIR_TIME_DELTA_SECONDS",
        "TIME_GAP_SECONDS", "IS_FIRST_INTERACTION_POINT", "INTERACTION_ID", "ASSET_PAIR"
    ]
    hme_df["TYPE"] = "HME"

    lv_df = df[[
        "LV_MACHINE", "LV_PREFIX", "HME_MACHINE", "HME_PREFIX",
        "TIMESTAMP", "DISTANCE_METRES", "INTERACTION_TYPE", "IS_NEAR_MISS",
        "IS_VEHICLE_INTERACTION", "MOVING_STATUS", "LV_IS_MOVING", "HME_IS_MOVING",
        "LV_X", "LV_Y", "LV_Z", "LV_GEOMETRY", "LV_FLEET_SIZE", "HME_FLEET_SIZE",
        "LV_LOCATION_TIMESTAMP", "HME_LOCATION_TIMESTAMP",
        "LV_SPEED_KPH", "HME_SPEED_KPH", "LV_SPEED_MPS", "HME_SPEED_MPS",
        "LV_MOVEMENT_METRES", "HME_MOVEMENT_METRES",
        "LV_MOVEMENT_3D_METRES", "HME_MOVEMENT_3D_METRES",
        "LV_TIME_DELTA_SECONDS", "HME_TIME_DELTA_SECONDS",
        "TIME_GAP_SECONDS", "IS_FIRST_INTERACTION_POINT", "INTERACTION_ID", "ASSET_PAIR"
    ]].copy()
...
def summarise_for_pareto(pareto_df, group_columns):
    ...
    summary = (
        ...
        .agg(
            ...
            AVG_HME_SPEED_KPH=("HME_SPEED_KPH", "mean"),
            MAX_HME_SPEED_KPH=("HME_SPEED_KPH", "max"),
            AVG_LV_SPEED_KPH=("LV_SPEED_KPH", "mean"),
            MAX_LV_SPEED_KPH=("LV_SPEED_KPH", "max"),
        )
        .reset_index()
    ...
    speed_columns = [
        "AVG_HME_SPEED_KPH", "MAX_HME_SPEED_KPH",
        "AVG_LV_SPEED_KPH", "MAX_LV_SPEED_KPH",
    ]
...
def build_rate_summary(pareto_df, hours):
    if pareto_df.empty:
        return pd.DataFrame(columns=[
            "HME_ASSET", "FLEET_SIZE", "VEHICLE_INTERACTIONS", "NEAR_MISSES",
            "AVG_HME_SPEED_KPH", "MAX_HME_SPEED_KPH", "AVG_LV_SPEED_KPH", "MAX_LV_SPEED_KPH",
...
def get_pareto_payload(start_date, end_date):
    ...
    out["HME_ASSET"] = out["PREFIX"].where(out["TYPE"].eq("HME"), out["PAIR_PREFIX"])
    out["LV_ASSET"] = out["PAIR_PREFIX"].where(out["TYPE"].eq("HME"), out["PREFIX"])
    out["AREA_CATEGORY"] = out["AREA_CATEGORY"].fillna("Unknown")
    out["ASSET_PAIR"] = out["HME_ASSET"].astype(str) + "-" + out["LV_ASSET"].astype(str)
    out["ASSET_AREA_CATEGORY"] = out["ASSET_PAIR"] + " | " + out["AREA_CATEGORY"]

    # The payload keeps one row per HME interaction. Expose clearly named HME/LV
    # speed and supporting calculation fields for summaries and Excel exports.
    out["HME_LOCATION_TIMESTAMP"] = out["LOCATION_TIMESTAMP"]
    out["LV_LOCATION_TIMESTAMP"] = out["PAIR_LOCATION_TIMESTAMP"]
    out["HME_SPEED_KPH"] = pd.to_numeric(out["SPEED_KPH"], errors="coerce")
    out["LV_SPEED_KPH"] = pd.to_numeric(out["PAIR_SPEED_KPH"], errors="coerce")
    out["HME_SPEED_MPS"] = pd.to_numeric(out["SPEED_MPS"], errors="coerce")
    out["LV_SPEED_MPS"] = pd.to_numeric(out["PAIR_SPEED_MPS"], errors="coerce")
    out["HME_MOVEMENT_METRES"] = pd.to_numeric(out["MOVEMENT_METRES"], errors="coerce")
    out["LV_MOVEMENT_METRES"] = pd.to_numeric(out["PAIR_MOVEMENT_METRES"], errors="coerce")
    out["HME_MOVEMENT_3D_METRES"] = pd.to_numeric(out["MOVEMENT_3D_METRES"], errors="coerce")
    out["LV_MOVEMENT_3D_METRES"] = pd.to_numeric(out["PAIR_MOVEMENT_3D_METRES"], errors="coerce")
    out["HME_TIME_DELTA_SECONDS"] = pd.to_numeric(out["TIME_DELTA_SECONDS"], errors="coerce")
    out["LV_TIME_DELTA_SECONDS"] = pd.to_numeric(out["PAIR_TIME_DELTA_SECONDS"], errors="coerce")

    # Loading operations to exclude from Pareto.
    # In the HME/LV query, WL is the loading-related HME asset.
    ...
    pareto_detail = out[out["TYPE"].eq("HME")].copy()
    pareto_detail = add_historical_risk_analysis(pareto_detail)

    keep_columns = [
        "TIMESTAMP_STR", "MACHINE", "PAIR_MACHINE", "HME_ASSET", "LV_ASSET", "ASSET_PAIR",
        "HME_LOCATION_TIMESTAMP", "LV_LOCATION_TIMESTAMP",
        "HME_SPEED_KPH", "LV_SPEED_KPH", "HME_SPEED_MPS", "LV_SPEED_MPS",
        "HME_MOVEMENT_METRES", "LV_MOVEMENT_METRES",
        "HME_MOVEMENT_3D_METRES", "LV_MOVEMENT_3D_METRES",
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            "Eliwana HME/LV Interaction Pareto",
            style={
                "fontSize": "24px",
                "fontWeight": "bold",
                "marginBottom": "15px",
        ...
        dcc.Checklist(
            ...
            options=[{
                "label": "Hide loading operation interactions: WL",
                "value": "hide_loading",
            }],
            value=[],
